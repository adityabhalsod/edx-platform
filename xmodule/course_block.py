...
class CourseFields:  # lint-amnesty, pylint: disable=missing-class-docstring
    ...
    enrollment_start = Date(
        ...
        default=(
            DEFAULT_START_DATE
            if CREATE_COURSE_WITH_DEFAULT_ENROLLMENT_START_DATE.is_enabled()
            else None
        ),
        scope=Scope.settings,
    )
    ...
    force_on_flexible_peer_openassessments = Boolean(
        ...
        help=_(
            "Setting this flag will force on the flexible grading option for all peer-graded ORAs in this course."
        ),
        scope=Scope.settings,
        default=False,
    )
...
class CourseBlock(
    ...
    def forum_posts_allowed(self):
        ...
        posting_restrictions = self.discussions_settings.get(
        ...
        return all(
            not (blackout["start"] <= now <= blackout["end"]) for blackout in blackouts
        )
